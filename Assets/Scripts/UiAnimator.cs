...
public class UiAnimator : MonoBehaviour {
    ...
    private SwarmController _swarmUiController;
    private bool _isVisible;
    private float _animationTimeOverride;
    private Vector3 _initalEnd;

    // private void Awake() {
    //     _swarmUiController = transform.parent.GetComponent<SwarmController>();
    ...
    public void Load(SwarmController swarmUiController) {
        _swarmUiController = swarmUiController;
        Assert.IsNotNull(_swarmUiController);
        _swarmUiController.OnShow += OnShow;
        _swarmUiController.OnHide += OnHide;
        _rectTransform = GetComponent<RectTransform>();
